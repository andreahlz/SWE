...
process calculate_coverage {
    ...
    python3 ${params.py_coverage_calc} \
        --input_file "${input_txt}" \
        --newheader_dir processed \
        --output_short coverage_short.txt \
        --output_long coverage_long.txt
    """
}
...
workflow{
    ...
    // Calculate coverage
    coverage_files = calculate_coverage(processed_fastas[0].collect(), input_ch)

    short_ch = short_reads(processed_fastas[1], input_ch, coverage_files[0])
    long_ch  = long_reads(processed_fastas[1], input_ch, coverage_files[1])
