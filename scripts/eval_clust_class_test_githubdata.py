    ...
    for model in model_list:
        for dataset in ["reference","marine","plant"]: #datasets. we only got one so far
            ...
            for reads_mode in [0,1,2,3,4]: 
                ...
                labels_data[f"true_ground_{dataset}_{reads_mode}"] = labels

                # generate embedding
                embedding = get_embedding(dna_sequences, model, dataset, reads_mode, test_model_dir=args.test_model_dir,path_data_dir="/home/barbara/evaluate")
                ...
                embeddings_all[f"low_dim_{dataset}_{reads_mode}"] = embedding_reduced
                embeddings_all[f"embedding_{dataset}_{reads_mode}"] = embedding
                embeddings_all[f"embedding_standard_{dataset}_{reads_mode}"] = embedding_standard
                ...
                for random_seed in random_seeds:
                    ...
                    labels_data[f"kmeans_predicted_{dataset}_{reads_mode}"] = preds_clustering
                    ...
                    ARI[f"{dataset}_{reads_mode}"] = ari
                ...
                print(lr_results)
                if f"{dataset}_{reads_mode}_labels_correct_predicted" not in colors_datapoints.keys():
            ...
            for dataset in ["reference","marine", "plant"]:
                ...
                for ind,reads_mode in enumerate([0,1,2,3,4]):
                    ...
                    for i in range(5):
                        dim1 = [embeddings_all[f"low_dim_{dataset}_{reads_mode}"][i][0] for i in range(len(embeddings_all[f"low_dim_{dataset}_{reads_mode}"]))]
                        dim2 = [embeddings_all[f"low_dim_{dataset}_{reads_mode}"][i][1] for i in range(len(embeddings_all[f"low_dim_{dataset}_{reads_mode}"]))]
...
if __name__ == "__main__":
    if DEBUG_MODE:
        ...
        args = argparse.Namespace(
            ...
            data_dir=f"/home/barbara/evaluate/dnabert-s_eval",
            model_list="test",            
        )
